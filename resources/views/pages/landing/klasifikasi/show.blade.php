    ...
    function klasifikasiSearch(klasifikasi) {
        return {
            ...
            renderResults(container, items) {
                ...
                const iconBuilding = `<svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/></svg>`;

                items.forEach(item => {
                    const card = document.createElement('div');
                    const produsenHtml = item.produsen
                        ? `<span class="flex items-center gap-1">${iconBuilding}${esc(item.produsen)}</span>`
                        : '';
                    ...
                    if (item.is_locked) {
                        ...
                        card.innerHTML = `
                            <div class="flex items-start sm:items-center gap-4 px-5 py-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    <h2 class="text-sm font-bold text-gray-800 line-clamp-1 mb-1 font-body">${esc(item.nama)}</h2>
                                    <div class="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-gray-400 font-body">
                                        ${item.klasifikasi ? `<span class="flex items-center gap-1"><svg class="w-3 h-3 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"/></svg>${esc(item.klasifikasi)}</span>` : ''}
                            ...
                            <div class="flex items-start sm:items-center gap-4 px-5 py-4">
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div class="flex flex-wrap gap-x-4 gap-y-1 text-[11px] text-gray-400">
                                        ${item.klasifikasi ? `<span>${esc(item.klasifikasi)}</span>` : ''}
                                        ${item.satuan_data ? `<span>${esc(item.satuan_data)}</span>` : ''}
                                        ${item.frekuensi_penerbitan ? `<span>${esc(item.frekuensi_penerbitan)}</span>` : ''}
