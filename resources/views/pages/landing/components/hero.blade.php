...
<section
    ...
    style="background: linear-gradient(135deg, #001020 0%, #001734 45%, #002a52 100%);"
    aria-label="Beranda"
>
    ...
    <div class="absolute inset-0 overflow-hidden pointer-events-none" aria-hidden="true">
        <div class="absolute top-1/4 -left-32 w-80 h-80 bg-[#3DB166]/10 blur-[80px]"></div>
        <div class="absolute bottom-1/4 -right-32 w-96 h-96 bg-[#3DB166]/8 blur-[100px]"></div>
    </div>
    ...
    <div class="relative z-10 max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 text-center pt-32 pb-24">
        ...
        {{-- Label badge --}}
        <div class="inline-flex items-center gap-2 px-4 py-1.5 bg-[#3DB166]/15 border border-[#3DB166]/40 mb-8 fade-up" style="animation-delay:0.1s">
            <span class="w-1.5 h-1.5 bg-[#3DB166] animate-pulse"></span>
            <span class="text-[#3DB166] text-xs font-bold tracking-widest uppercase">Platform Layanan Satu Data</span>
        </div>

        {{-- Heading --}}
        <h1 class="text-4xl sm:text-5xl lg:text-6xl font-poppins font-black text-white leading-tight tracking-tight mb-2 fade-up"
        ...
        <div class="relative z-60 max-w-2xl mx-auto mb-8 fade-up" style="animation-delay:0.4s"
             x-data="heroSearch()">
            <div class="relative">
                <div class="flex items-center bg-white shadow-2xl shadow-black/50 overflow-visible border-l-4 border-[#3DB166]">
                    <div class="pl-5 pr-3 shrink-0">
                        ...
                        <svg x-show="loading" class="w-5 h-5 text-[#3DB166] animate-spin" fill="none" viewBox="0 0 24 24" aria-hidden="true">
                    ...
                    <button
                        @click="goToSearch"
                        class="m-0 px-6 py-4 bg-[#001734] text-white text-sm font-bold transition-colors duration-200 shrink-0 hover:bg-[#3DB166] self-stretch flex items-center">
                        Cari
                    </button>
                </div>
                ...
                <div x-show="showSuggestions"
                     ...
                     @click.outside="close"
                     class="absolute top-full left-0 right-0 mt-0 bg-white shadow-2xl border border-gray-100 border-t-2 border-t-[#3DB166] overflow-hidden overflow-y-auto max-h-80 z-[999]"
                     role="listbox">
                    ...
                    <template x-for="item in suggestions" :key="item.metadata_id">
                        <a :href="`/klasifikasi/${slugify(item.klasifikasi)}`"
                           class="flex items-center gap-4 px-5 py-3.5 hover:bg-[#3DB166]/8 border-b border-gray-50 last:border-0 transition-colors cursor-pointer border-l-2 border-l-transparent hover:border-l-[#3DB166]"
                           role="option">
        ...
        <div class="relative z-10 flex flex-wrap justify-center gap-2 mb-12 fade-up" style="animation-delay:0.5s">
            @foreach($klasifikasiAktif as $k)
                <a href="{{ route('klasifikasi.show', ['klasifikasi' => \Illuminate\Support\Str::slug($k->nama_klasifikasi)]) }}"
                   class="px-3.5 py-1.5 bg-white/8 border border-white/15 text-white/70 text-xs font-medium hover:bg-[#3DB166]/20 hover:text-[#3DB166] hover:border-[#3DB166]/50 transition-all duration-200 backdrop-blur-sm">
                    {{ $k->nama_klasifikasi }}
                </a>
            @endforeach
        </div>
        ...
        <div class="flex flex-col sm:flex-row items-center justify-center gap-4 fade-up" style="animation-delay:0.6s">
            <a href="{{ route('data.index') }}"
               class="group inline-flex items-center gap-2 px-8 py-3.5 bg-[#3DB166] text-white font-black text-sm transition-all duration-200 shadow-xl hover:bg-[#2d9955] hover:shadow-[#3DB166]/30 hover:-translate-y-0.5">
        ...
        {{-- <div class="mt-16 grid grid-cols-3 gap-0 max-w-lg mx-auto fade-up border border-white/10 divide-x divide-white/10" style="animation-delay:0.7s">
            ...
            <div class="px-6 py-4 text-center">
                <div class="text-2xl font-black text-[#3DB166] font-poppins">{{ number_format($jumlahMetadata ?? 0) }}</div>
    ...
    <div class="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-transparent via-[#3DB166] to-transparent" aria-hidden="true"></div>
</section>
